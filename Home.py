...
#🛑 Code to create a spinner whenever this metric is being calculated. Everything that is contained on the metric 
#under the spinner needs to be idented, like it would be on a function
...
with st.expander(f"New Sets of the Top {filt_n_themes} Master Themes",expanded=True): # Replace ... by the correct method. 🛑Remember that everything contained on the expander needs to be idented.
    with st.spinner('Loading...'): #🛑Everything covered by the spinner needs to be idented.
        ...
        df_themes_to_parts = df_filtered.groupby(['parent_theme_name']).set_name.nunique().reset_index().rename(columns={'set_name':'nbr_sets'})
        ...
        fig_parent_theme = px.bar(sets_per_parent_theme,
            ...
            title=f'Number of Distinct Lego Sets per Parent Theme (Top {filt_n_themes})',
